...
# Path to the parent folder of `core`
PROJECT_ROOT = Path("/Users/dimi/Downloads/my stuff/geometric")
...
if dsystem == 'LC':
    def f(x,mu=1,t=0):#Van der Pol oscillator
...
#     dim = 2
...
elif dsystem == 'Duffing':
    def f(x,a=0,b=-1.5,c=0.2, t=0):
        # Duffing oscillator
        x1_dot = x[1]
        x2_dot =  a*x[1] - x[0] * (b + c * x[0]**2)
    ...
    def f(x, t=0):
        x1_dot = -x[0] + 0.08 * x[1] + x[0]**2 * x[1]
        x2_dot = 0.6 - 0.08 * x[1] - x[0]**2 * x[1]
        return np.array([x1_dot, x2_dot])

    x0 = np.array([0.7, 1.25])
    dim = 2
...
#         x1_dot = 0.5 * x[0] + x[1] - x[0] * r_squared
#         x2_dot = -x[0] + 0.5 * x[1] - x[1] * r_squared
#         return np.array([x1_dot, x2_dot])
...
gpf.config.set_default_float(tf.float64)   # or tf.float64, but be consistent
DTYPE = gpf.default_float()

def simulate_path_numpy(f_drift_vec, x0, timegrid, g, dt):
    """
    Vectorised variant *only if* f_drift_vec accepts (N, dim) and returns (N, dim),
    and you are OK computing the drift on the already-known X_{t-1} states.
    """
    ...
    for t in range(T - 1):
        # pass (1, D) to the GP
        drift = f_drift_vec(X[t:t+1])[0]  # (D,)
        noise = 0.0
        if sigma is not None:
            eps = rng.normal(size=D)
            if np.ndim(g) == 0:          # scalar g
                noise = g * sigma * eps * sqrt_dt
            else:                         # matrix g
                noise = (g @ (sigma * eps)) * sqrt_dt
...
@torch.no_grad()
def simulate_path_torch(f_drift_torch, x0, timegrid, g, h, device="cpu"):
    """
    f_drift_torch(torch.Tensor) : (N,2) → (N,2)   (latent drift, no noise)
    """
    x0 = torch.tensor(x0, dtype=torch.float64, device=device)
    T  = len(timegrid)
    dim, = x0.shape
    F = torch.empty(T, dim, device=device)
    F[0] = x0
    ...
    for ti in range(1, T):
        drift = f_drift_torch(F[ti-1:ti])[0]      # keep batch dim
        noise = g_t * torch.randn(dim, device=device) * sqrt_h
        F[ti] = F[ti-1] + h * drift + noise

    return F.T.cpu().numpy()      # back to (2,T) NumPy like before



def simulate_path(f_drift,x0, timegrid, the_g, h):
...
# def to_float32(x):
#     """
#     Converts numpy arrays, lists, or TF tensors to float32.
#     """
...
def GP_drift_estimation( X, dt, nsparse=200, lengthscales=0.1, variance=0.1, multirep=False, rep_keep=1):
    ...
    # after you create mf0
    # GPflow ≥2.2
    gpf.utilities.set_trainable(mf0.inducing_variable, True)
    # or, explicitly:
    gpf.utilities.set_trainable(mf0.inducing_variable.Z, True)
    
    # # (optional) verify
    # for v in mf0.trainable_variables:
    #     if "inducing_variable" in v.name:
    #         print(v.name, v.trainable)  # should be False



    opt =gpf.optimizers.Scipy()
    opt_logs = opt.minimize(mf0.training_loss, mf0.trainable_variables, options=dict(maxiter=400))
...
def evaluation_for_given_drift(f_given, f_givenvar, ztrue, the_timegrid,
                               obs_den, F_obs, bounds, g,x0, etype='pure'):
    ...
    # else:
    var = 0
    #     (gridX, gridY), mu, var = evaluate_vector_field_gpflow(
    #         posterior, n=n_grid, bounds=bounds, return_var=True, chunk=20000)
    xrid, mu = evaluate_vector_field( f_given, etype= 'pure', n=n_grid, bounds=bounds)
    ...
    #     F_sim = simulate_path(f_given, the_timegrid, g, dt)
    ...
    F_sim = simulate_path_numpy(f_given,x0, the_timegrid, g, dt)
...
def evaluate_vector_field_gpflow(posterior, n, bounds, return_var=True,
                                 dtype=tf.float64, chunk=None):
    """
    posterior : gpflow posterior (e.g. model.posterior())
    n         : grid resolution per axis
    bounds    : [(x_min, x_max), (y_min, y_max)]
    return_var: whether to return variance
    chunk     : int or None; if set, do chunked prediction to save RAM
    """
    # build grid (n*n, 2)
    xs = np.linspace(bounds[0][0], bounds[0][1], n, dtype=np.float64)
    ys = np.linspace(bounds[1][0], bounds[1][1], n, dtype=np.float64)
    Xg, Yg = np.meshgrid(xs, ys, indexing="xy")
    X = np.stack([Xg.ravel(), Yg.ravel()], axis=1)

    X_tf = tf.convert_to_tensor(X, dtype=dtype)

    if chunk is None:
        mean, var = posterior.predict_f(X_tf)
        mean = mean.numpy()   # (n*n, D)
        var  = var.numpy() if return_var else None
    else:
        outs_m, outs_v = [], []
        for i in range(0, X.shape[0], chunk):
            Xi = tf.convert_to_tensor(X[i:i+chunk], dtype=dtype)
            mi, vi = posterior.predict_f(Xi)
            outs_m.append(mi.numpy())
            if return_var:
                outs_v.append(vi.numpy())
        mean = np.vstack(outs_m)
        var  = np.vstack(outs_v) if return_var else None

    D = mean.shape[1]
    mean = mean.T.reshape(D, n, n)                       # (D, n, n)
    if return_var:
        var = var.T.reshape(D, n, n)
        return (Xg, Yg), mean, var
    else:
        return (Xg, Yg), mean
...
def evaluation_for_given_drift(f_given, f_givenvar, ztrue, the_timegrid,
    ...
    ##Evaluate vector field 
    # if etype=='est':
    #     _, mu, var = evaluate_vector_field( f_given, etype= etype, n=n_grid, bounds=bounds)
    # else:
    var = 0
    # (gridX, gridY), mu, var = evaluate_vector_field_gpflow(
    #     posterior, n=n_grid, bounds=bounds, return_var=True, chunk=20000)
    ...
    if etype=='est':
        ...
        F_sim = simulate_path(f_given, x0,the_timegrid, g, dt)
    return mu, var, RMSE, F_sim, was_mean, was_std
...
def score_function_multid_seperate_all_dims(X,Z,func_out=False, C=0.001,kern ='RBF',l=1,which=1):
    ...
    if isinstance(l, (list, tuple, np.ndarray)):
       ...
       #K_xz =  np.ones((X.shape[0],Z.shape[0]), dtype=np.float32) 
       #Knumba(X,Z,l,K_xz,multil=True) 
       K_xz = K(X,Z,l,multil=True) 
       #Ks =  np.ones((Z.shape[0],Z.shape[0]), dtype=np.float32) 
                ...
                while sinx.size>0 and counter<100:   
                    ...
                    for ti,tt in enumerate(self.timegrid[:-1]):
                        ...
                        if ti==0:
                            ...
                            # print('B shape', self.B[:,sinx,rev_ti+1].shape)   
                            # print('f', (self.f(self.B[:,sinx,rev_ti+1], self.timegrid[rev_ti]) ).shape)
                            # print('grad log',  (self.g**2*grad_ln_ro[:,sinx]).shape   )
                            pBi = self.B[:,sinx,rev_ti+1].reshape((self.dim, -1))
                            fBi = self.f(pBi , self.timegrid[rev_ti]).reshape((self.dim, -1))
                            
                            
                            self.gbB[:,sinx,rev_ti+1] =  -fBi + self.g**2*grad_ln_ro[:,sinx].reshape((self.dim, -1))   
                            if (ti>=1) :#False:#callable(self.grad_log_pss):
                                  
                              self.B[:,sinx,rev_ti] = pBi - fBi*self.dt + \
                              self.dt*self.g**2*grad_ln_ro[:,sinx].reshape((self.dim, -1)) + \
        ...
        while sinx.size>0 and counter<100: 
            ...
            for ti, tt in enumerate(self.timegrid[:-1]):
                ...
                if ti==0:
                    ...
                    #### secure reshaping to acount for single particle call dimension problems
                    fFt = self.f(self.Fc[:,sinx,ti-1]).reshape((self.dim,-1)) #f of previous step
                    pFc = self.Fc[:,sinx,ti-1].reshape((self.dim,-1)) # previous step
                    
                    self.gfFc[:, sinx, ti] = fFt + self.g**2 *uu1#.T
                    self.Fc[:, sinx, ti] =  pFc + dtt* fFt +dtt*self.g**2 *uu1\
...
def construct_geodesic(start, stop, mani_samples, num_t, omit=None, N_nodes=10,
    ...
    manifold = manifolds.LocalDiagPCA(data=data, sigma=sigma, rho=1e-3)
...
import pickle
from scipy.spatial.distance import cdist
def compute_bridge_covariances(F, unbiased=False):
    """
    F: np.ndarray of shape (d, N, T)

    returns:
        covs: np.ndarray of shape (T, d, d)

    At each time t:
        covs[t] = covariance of particles in R^d
    """
    d, N, T = F.shape
    covs = []
    ...
    for t in range(T):
        X = F[:, :, t]                         # shape (d, N)
        mu = np.mean(X, axis=1, keepdims=True)  # shape (d, 1)
        Xc = X - mu                           # centered particles

        denom = (N - 1) if unbiased and N > 1 else N
        C = (Xc @ Xc.T) / denom               # shape (d, d)
        covs.append(C)

    return np.stack(covs, axis=0)             # shape (T, d, d)

#   runs ONE bridge and returns the slice we need
#   must live at module level so that it is pickle-able
# ----------------------------------------------------------------------
def _compute_bridge(tindx,
                    timegrid_obs, Ob,                # global arrays
                    bridge_type, f_est, g, f_true,   # models
                    ksteps, N, M, dt,                # scalars
                    distances=None, epsilon_ball=0.0, strength=1.0,
                    save_folder=''):
    """
    Run BRIDGE_ND_reweight for a single observation window.

    Returns
    -------
    tindx         : int
    Fc_slice      : np.ndarray     shape (dim, dim, ksteps-3)
    gfFc_slice    : np.ndarray     shape (dim, dim, ksteps-3)
    valid         : bool
    """
    save_bridge=False
    if tindx in [4,5,6]:
        save_bridge=True
        save_title = 'bridges_num_%d_beta_%.2f.dat'%(tindx, strength)
    # 1-thread BLAS inside each worker to avoid oversubscription
    with threadpool_limits(limits=1):
        ...
        phase = np.arctan2(Ob[1], Ob[0])
        phase = (phase + 2*np.pi) % (2*np.pi)
        
        # corrected mask for clockwise arc
        def is_on_clockwise_arc(phase, ph1, ph2):
            return (np.mod(phase - ph2, 2*np.pi) < np.mod(ph1 - ph2, 2*np.pi))
        
        
        try:
            ### first construct  true bridge
            true_bridg = BRIDGE_ND_reweight(t1, t2, y1, y2, f_true, g, N, M, dens_est='nonparametric',
                                        reject=True, plotting=True, sample='deterministic', dt=dt)
            print('The true bridge is valid:', true_bridg.valid)
            # choose bridge type ----------------------------------------------
            if bridge_type == "normal":
                ...
                if dsystem=='Hopf':
                    ...
                    ph1 = phase[tindx]
                    ph2 = phase[tindx+1]
                    to_keep = is_on_clockwise_arc(phase, ph1, ph2)
                elif tindx>0:
                ...
                geodesic_fun = construct_geodesic(y1, y2, Ob[:,to_keep], timegrid_obs.size,
                                                  omit=None, N_nodes=10, plotting=to_plot)          
                ...
                bridg2d = BRIDGE_ND_reweight(t1, t2, y1, y2, f_est, g, N, M, dens_est='nonparametric',
                                        reject=True, plotting=True, reweight=True, U=Ugeod2 ,uncertend=-3, dt=dt)
                ...
                if bridg2d.valid:
                    ...
                    bridg2d = BRIDGE_ND_reweight(t1, t2, y1, y2, f_br, g, N, M,
                                                 dens_est='nonparametric',
                                            reject=True, plotting=True, reweight=True,
                                            U=Ugeod2,uncertend=-3, dt=dt)
                    
                    print('The bridge is valid:', bridg2d.valid)
            if not bridg2d.valid:           # return empty arrays so caller can skip
                return tindx, None, None, False, None

            # Fc_slice   = bridg2d.Fc[:, :, 2:-1]
            # gfFc_slice = bridg2d.gfFc[:, :, 2:-1]
            
            Fc_slice   =  bridg2d.B[:, :, 2:-1]
            gfFc_slice = -bridg2d.gbB[:, :, 2:-1]
            
            #################################################################
            #### Evaluate quality of bridges compared to true ones
            #################################################################
            ## compute mean difference between true bridge and approximate one
            true_mean   = np.mean(true_bridg.Fc, axis=1)
            approx_mean = np.mean(bridg2d.Fc   , axis=1)
            
            diff = true_mean - approx_mean    # (d, T)
            mean_diff_t = np.linalg.norm(diff, axis=0) # (T,)
            mean_diff_avg = mean_diff_t.mean().item()
            #######################################################################
            ## compute covariance difference between true bridge and approximate one
            C_true   = compute_bridge_covariances(true_bridg.Fc, unbiased=False) # (T, d, d)
            C_approx = compute_bridge_covariances(bridg2d.Fc, unbiased=False)    # (T, d, d)
        
            diff = C_true - C_approx                                             # (T, d, d)
            cov_diff_t = np.linalg.norm(diff, ord='fro', axis=(1, 2))   # (T,)
            cov_diff_avg = cov_diff_t.mean().item()
            
            ##########################################
            ##compute Wasserstein-2 distance over time
            d1, N1, T1 = true_bridg.Fc.shape
            d2, N2, T2 = bridg2d.Fc.shape
        
            if d1 != d2 or T1 != T2:
                raise ValueError("F_true and F_approx must have same state dimension and same number of time steps.")
            if N1 != N2:
                raise ValueError("This exact matching implementation assumes the same number of particles in both bridges.")
            ...
            for t in range(T):
                X = true_bridg.Fc[:, :, t].T    # shape (N, d)
                Y = bridg2d.Fc[:, :, t].T  # shape (N, d)
        
                # squared Euclidean cost matrix, shape (N, N)

                C_np = cdist(X, Y, metric='euclidean') ** 2
                
                # Hungarian algorithm expects numpy
                row_ind, col_ind = linear_sum_assignment(C_np)
        
                # exact empirical W2^2 with equal weights
                w2_sq = C_np[row_ind, col_ind].sum() / N
                w2 = np.sqrt(w2_sq)
        
                w2_list.append(w2)
        
            w2_t = np.array(w2_list)
            w2_avg = w2_t.mean().item()
            
            bridge_dists = dict()
            bridge_dists['w2_t']   = w2_t
            bridge_dists['w2_avg'] = w2_avg
            
            bridge_dists['cov_diff_t']   = cov_diff_t
            bridge_dists['cov_diff_avg'] = cov_diff_avg
            
            bridge_dists['mean_diff_t']   = mean_diff_t
            bridge_dists['mean_diff_avg'] = mean_diff_avg
            
            
            if save_bridge:
                # create dict
                bridge_dict = { "true_bridg_Fc": true_bridg.Fc,
                                "bridg2d_Fc"   : bridg2d.Fc,   }
                
                # save to pickle file
                with open(save_folder+save_title, "wb") as f:
            ...
            if tindx%4==0:
                ...
                plt.plot(bridg2d.Fc[0,:,:].T,bridg2d.Fc[1,:,:].T,alpha=0.53, c='maroon')
                plt.plot(Ob[0],Ob[1],'.') 
                plt.plot(y1[0],y1[1],'v') 
                plt.plot(y2[0],y2[1],'x')
                plt.savefig( save_dirplots+f'Bridge_for_{t1}_{strength}.png')
                plt.show()
                plt.close()
            return tindx, Fc_slice, gfFc_slice, True, bridge_dists
...
def run_bridge_augmentation_parallel(f_est, bridge_type,f_true, ksteps, N, M, reps,aug_num,
                                     strength=1, dt=0.01,savetitle='',save_dir=''):
    """
    ...
    aug_num: int, augmentation number in the whole inference process. this is to determini which paralle backend to use
    ...
    n_jobs = 3                                        
    num_bridges = 50#0
    indices = range(min(num_bridges, timegrid_obs.size - 1))
    # parallel execution ------------------------------------------------
    ### the first parallel loop has to run with threading backend due to 
    ### issues with pickling the the @tf.function for fast drift evaluation
    backends_for_paral = ["threading", "loky", "loky", "loky", "loky", "loky"]
    the_backend = backends_for_paral[aug_num]
    
    with tqdm_joblib(tqdm(total=len(indices), desc="bridges", ncols=100)) as _:
        results = Parallel(n_jobs=n_jobs, backend=the_backend)(
            delayed(_compute_bridge)(i, timegrid_obs, Ob, bridge_type, f_est, g,f_true,
                ksteps, N=N, M=M, dt=dt, distances=distances, epsilon_ball=epsilon_ball,
                strength=strength, save_folder=save_dir)  for i in indices )

    # ------------------------------------------------------------------
    # copy slices back into BALL / gbALL in the *main* process
    #    (avoids concurrent writes & shared-memory headaches)
    # ------------------------------------------------------------------
    brdistances = dict()
    
    for (tindx, Fc_slice, gfFc_slice, valid, bridge_dict) in results:
        if not valid:
            print(f"skipping bridge {tindx} (invalid or overflow)")
            continue

        outer1 = tindx * ksteps
        outer2 = (tindx + 1) * ksteps
        BALL[:, :, outer1 + 2 : outer2]  = Fc_slice
        gbALL[:, :, outer1 + 2 : outer2] = gfFc_slice

        brdistances[tindx] = bridge_dict
    
    return BALL, gbALL, brdistances  




def run_bridge_augmentation(f_est, bridge_type, ksteps, N, M, reps, strength=1, dt=0.01,savetitle=''):
    """
    f_est : estimated drift for bridge sampling
    bridge_type : string, determines what kind of bridge will be employed
                  options:
    ...
    to_keep = np.ones(Ob.shape[1], dtype=bool)
    epsilon_ball = 0.55
    for tindx, tim in enumerate(timegrid_obs[:50]): ##loop over the observation coarse timegrid
      if tindx>=0:
          print('------------------------- \n')
          ...
          try:
              ...
              print(BALL[:,:,tindx_outer1+2:tindx_outer2].shape)
              print(bridg2d.dt)
...
import os
sys.path.append('/Users/dimi/Downloads/my stuff/geometric/core')
Ts = [ 300, 500]#, 1500] ,200, 300,400
obs_denz = [280] #, 280 ,  160,200, 240,280 160, 
#obs_denz = [160, 200, 240, 280]#[80,120,160,200, 240, 280,  320]
#seed = 10
seeds = [  13]#,11,12,13,14] #12 sta 200 13, 14  ,11,12,13,14, 13, ,14 
for seed in seeds:
    # obs dens 200, 300, 400 at 0.15 k 0.25
     #, 240, 280
    for obi, obs_dens in enumerate(obs_denz[::-1]):
        for T in Ts:
            plt.close('all')
            
            
            #int(sys.argv[1]),int(sys.argv[2]),int(sys.argv[3])
            #00#Ts[Ti]  #length of simulation
            #obs_dens = obs_denz[0]#obs_denz[obi] #interval between successive observations
            g = 0.5#diffusion_s #noise amplitude
            np.random.seed(seed)
            ...
            first_augm = 'geodesic' # 'normal'
            
            timegrid = np.arange(0,T,h)
            timegrid_obs = timegrid[::obs_dens]  
            
            save_dir = '/Volumes/KINGSTON/geometric_more/VanDerGeodesic_augmentation_'+dsystem+'_noise_%.2f_obs_dens_%d_sim_time_%d_seed_%d/'%(g, obs_dens, T, seed)
            save_dirplots = '/Volumes/KINGSTON/geometric_more/VanDerGeodesic_augmentation_'+dsystem+'_noise_%.2f_obs_dens_%d_sim_time_%d_seed_%d/plots/'%(g, obs_dens, T, seed)
            
            
            if os.path.exists(save_dir) == False:
                os.mkdir(save_dir)
            if os.path.exists(save_dirplots) == False:
                os.mkdir(save_dirplots)
            ...
            with open(save_dir+"output.txt", "a") as fi:
                ...
                print('Problem characterisitcs: \n Simulation duration: %d \n Observation interval: %d \n Noise amplitude: %.2f \n Simulation precision: %.4f'%(T, obs_dens, g, h))
                print('Problem characterisitcs: \n Simulation duration: %d \n Observation interval: %d \n Noise amplitude: %.2f \n Simulation precision: %.4f'%(T, obs_dens, g, h), file=fi)
                print('Seed:  %d '%(seed))
                print('Seed:  %d '%(seed), file=fi)
            
            F = simulate_path(f,x0,timegrid, g, dt)
            ...
            DTYPE = gpf.config.default_float()  # should be tf.float32 if you set it so
            
            tf.config.optimizer.set_jit(True)  # XLA globally
            
            # always derive both TF and NumPy dtypes from gpflow's default
            TF_DTYPE = tf.as_dtype(gpf.default_float())          
            ...
            def _to_ND(x, D):
                """Return X (N,D) and a flag saying whether caller used (D,N)."""
                x = np.asarray(x, dtype=NP_DTYPE)
                x = np.squeeze(x)
                if x.ndim == 1:
                    if x.shape[0] != D:
                        raise ValueError(f"expected length-{D} vector, got {x.shape}")
                    return x[None, :], False
                if x.ndim == 2:
                    if x.shape[1] == D:         # (N, D)
                        return x, False
                    if x.shape[0] == D:         # (D, N) -> (N, D)
                        return x.T, True
                raise ValueError(f"expected (D,), (N,D) or (D,N); got {x.shape}")
            
            def _restore_layout(arr, transposed):
                return arr.T if transposed else (arr[0] if arr.shape[0] == 1 else arr)
            
            posterior = mf_init.posterior()
            print('after posterior')
            
            @tf.function(jit_compile=True, input_signature=[tf.TensorSpec(shape=[None, 2], dtype=TF_DTYPE)])
            def _predict_tf(X):
                mean, _ = posterior.predict_f(X)
                return mean
            
            def f_init(xo, t=0):
                X, was_T = _to_ND(xo, D)
                X = tf.convert_to_tensor(X, dtype=TF_DTYPE)
                m = _predict_tf(X).numpy()
                return _restore_layout(m, was_T)
            
            def f_initvar(xo, t=0):
                X, was_T = _to_ND(xo, D)
                X = tf.convert_to_tensor(X, dtype=TF_DTYPE)
                mean, var = posterior.predict_f(X)
                m, v = mean.numpy(), var.numpy()
                return _restore_layout(m, was_T), _restore_layout(v, was_T)
            
            from sklearn.neighbors import KernelDensity
            from sklearn.model_selection import GridSearchCV
    
    
            mu_init, var_init, RMSE_init, F_init, Wm_init, Wstd_init = evaluation_for_given_drift(f_init, f_init, ztrue, timegrid[:100000], obs_dens, 
                                                                                                  Ob, bounds=bounds,g=g,x0=Ob[:,0],
                                                                                                  etype='pure')
            print('after eval drift')
            params = {"bandwidth": np.logspace(-1, 1, 20)}
            grid1 = GridSearchCV(KernelDensity(), params)
            grid1.fit(Ob.reshape(dim, -1).T)
            ...
            wRMSE_init = wcompute_mean_squared_error(ztrue.reshape(dim, -1),
                                                      ...
                                                      weights)
            print('seed',seed, 'g', g, obs_dens, 'wRMSE init',wRMSE_init)
            # with open(save_dir+"output.txt", "a") as fi:    
            ...
            # # choose two points to define the clockwise arc
            # f1_idx = 19  # index of starting point
            # f2_idx = 20  # index of ending point
            # f1 = phase[f1_idx]
            # f2 = phase[f2_idx]
            # # corrected mask for clockwise arc
            # def is_on_clockwise_arc(phase, f1, f2):
            #     return (np.mod(phase - f2, 2*np.pi) < np.mod(f1 - f2, 2*np.pi))
            
            # mask_corrected = is_on_clockwise_arc(phase, f1, f2)
            
            # # plotting corrected version
            # fig, axes = plt.subplots(1, 2, figsize=(12, 6))
            
            # # left: all observations with phase coloring
            # sc1 = axes[0].scatter(Ob[ 0], Ob[ 1], c=phase, cmap='hsv', s=20)
            # axes[0].scatter(Ob[0,f1_idx], Ob[1,f1_idx], c='green', s=100, marker='o', label='f1 (start)')
            # axes[0].scatter(Ob[0,f2_idx], Ob[1,f2_idx], c='k', s=100, marker='X', label='f2 (end)')
            # axes[0].set_title('all observations colored by phase')
            # axes[0].axis('equal')
            
            # # right: filtered using corrected clockwise arc logic
            # sc2 = axes[1].scatter(Ob[0,mask_corrected], Ob[1,mask_corrected], c=phase[mask_corrected], cmap='hsv', s=20)
            # axes[1].scatter(Ob[0,f1_idx], Ob[1,f1_idx], c='green', s=100, marker='o', label='f1 (start)')
            # axes[1].scatter(Ob[0,f2_idx], Ob[1,f2_idx], c='red', s=100, marker='X', label='f2 (end)')
            # axes[1].set_title('filtered observations (clockwise f1 → f2)')
            # axes[1].axis('equal')
            # axes[1].legend()
            
            # fig.colorbar(sc1, ax=axes[0], label='phase (rad)')
            # fig.tight_layout()
            # plt.show()
            
            #########
            def plot_estimated_vector_field_and_invariant_density(xt, ztrue, mu, F_obs,  savetitle=''):
                ...
                plt.subplot(1,2,2)
                #plt.plot(F[0],F[1],'.')
                plt.plot(F_obs[0,:],F_obs[1,:],'y-',alpha=0.25)
                plt.plot(F_obs[0,:],F_obs[1,:],'r.')
                plt.title('True invariant density')
                
                plt.savefig(save_dir+'Estimated_force_field'+savetitle+'.png')
                plt.savefig(save_dir+'Estimated_force_field'+savetitle+'.pdf')
                plt.show()
                plt.close()
                
            plot_estimated_vector_field_and_invariant_density(xt, ztrue, mu_init, Ob,  savetitle='_initial')
            
            
            for strength in strengths:
                print('>>>>>>>>>>>>>>>>>>>>>>>>>>> currently at strength:', strength)
                
                dim = 2
                reps = 10
                M = 80 # num of inducing points for the grad log estimation
                dt = h
                augmentation_num = 0
                ...
                # else:
                bridge_types = [first_augm, 'normal', 'geodesic', 'geodesic', 
                ...
                f_true = f
                dists_per_augmentation = dict()
                while augmentation_num<2:
                    N = Ns
                    bridge_type = bridge_types[augmentation_num]
                    
                    if augmentation_num==0:
                        BALL, gbALL, brdistances = run_bridge_augmentation_parallel(f_init, bridge_type, f_true, ksteps, 
                                                                      N, M, reps,
                                                                      aug_num=augmentation_num ,
                                                                      strength=strength,
                                                                      dt = dt,
                                                                      savetitle='Augmentation_num_%d_augm_type_%s_beta_%.2f'%(augmentation_num, bridge_type, strength),
                                                                      save_dir=save_dir)
                    else:
                        BALL, gbALL, brdistances = run_bridge_augmentation_parallel(f_est, bridge_type, f_true, ksteps, 
                                                                      N, M, reps,
                                                                      aug_num=augmentation_num ,
                                                                      strength=strength,
                                                                      dt = dt,
                    ...
                    if seed==15:
                        filehandler = open(save_dir+"bridges_from_%d_augmentation_beta_%.2f.dat"%(augmentation_num, strength),"wb")
                        to_sav = dict()
                        to_sav['BALL'] = BALL
                        to_sav['gbALL'] = gbALL    
                        to_sav['brdistances'] = brdistances
                    ...
                    ##compute lenthscales###############################
                    if OPTIMIZE_LENGTHSCALES:
                        def _one_like(i, j, l1, l2, Sp, n_sparsegp, BALL2_view, gbALL2_view):
                            # avoid BLAS oversubscription inside each process
                            with threadpool_limits(limits=1):
                                # assuming GP_path_likelihood returns: _, _, like, ...
                                _, _, like, _, _, _, _, _ = GP_path_likelihood(
                                    Sp, n_sparsegp, BALL2_view, gbALL2_view, l1, l2
                                )
                            return i, j, like
                    
                        def run_likelihood_grid(Sp, n_sparsegp, BALL2, gbALL2,l1s,
                                                l2s, tmax=1000, n_jobs=4, save_dir=None):
                            # pre-slice once so workers don’t slice repeatedly
                            BALL2_view  = BALL2[:, :, :tmax]
                            gbALL2_view = gbALL2[:, :, :tmax]
                        
                            all_like = np.empty((l1s.size, l2s.size), dtype=float)
                        
                            tasks = [(i, j, l1, l2) for i, l1 in enumerate(l1s)  for j, l2 in enumerate(l2s)]
                        
                            # progress bar
                            with tqdm_joblib(tqdm(total=len(tasks), desc="likelihood grid", ncols=100)):
                                results = Parallel(n_jobs=n_jobs, backend="loky")(
                                    delayed(_one_like)(i, j, l1, l2, Sp, n_sparsegp, BALL2_view, gbALL2_view)
                                    for (i, j, l1, l2) in tasks)
                            # write back
                            for i, j, like in results:
                                all_like[i, j] = like
                        
                            # log (only in parent)
                            if save_dir is not None:
                                with open(Path(save_dir) / "output.txt", "a") as fi:
                                    for i, l1 in enumerate(l1s):
                                        print(f"l1: {l1:.3f}", file=fi)
                        
                            l1ind, l2ind = np.unravel_index(np.argmin(all_like), all_like.shape)
                            return all_like, (l1ind, l2ind), (l1s[l1ind], l2s[l2ind])
                            
                        
                        l1s = np.arange(0.5, 10.5, 1.)
                        l2s = np.arange(0.5, 10.5 , 1.)
                        
                        all_like, (l1ind, l2ind), (best_l1, best_l2) = run_likelihood_grid(Sp, n_sparsegp, BALL2, gbALL2,l1s,l2s)
                        l1ind, l2ind = np.unravel_index(np.argmin(all_like), all_like.shape)
                        l1best = l1s[l1ind]
                        l2best = l2s[l2ind]
                        
                        print("Best l1:", l1best, " Best l2:", l2best)
                        ###############################################################
                    else:
                        l1best = 1.#l1s[l1ind]
                        l2best = 1.#l2s[l2ind]        
                    ...
                    f_est1, f_est2, ll, Ls, ds, I1,I2, lnth = GP_path_likelihood(Sp, n_sparsegp, BALL2,
                                                                                 gbALL2,l1best,
                                                                                 l2best)
                    ...
                    filehandler = open(save_dir+"f_est_after_%d_augm_beta_%.2f.dat"%(augmentation_num, strength),"wb")
                    ...
                    mu_1, var_1, RMSE_1, F_1, Wm_1, Wstd_1 = evaluation_for_given_drift(f_est, f_est, ztrue, timegrid[:100000],
                                                                                        obs_dens, Ob, bounds=bounds, g=g,x0=Ob[:,0],
                                                                                         etype='pure')
                    
                    wRMSE_1 = wcompute_mean_squared_error(ztrue.reshape(dim, -1),
                                                              mu_1.reshape(dim, -1),
                                                              weights)
                    print('seed',seed, 'g', g, obs_dens, 'wRMSE aug',wRMSE_1)
                
                    filehandler = open(save_dir+"f_est_after_%d_augm_n_eval_beta_%.2f.dat"%(augmentation_num, strength),"wb")
                    to_sav = dict()
                    to_sav['ll'] = ll
                    to_sav['Ls'] = Ls
                    to_sav['ds'] = ds
                    ...
                    plot_estimated_vector_field_and_invariant_density(xt, ztrue, 
                                                                      mu_1, Ob,  
                                                                      savetitle='_augm%d_beta_%.2f'%(augmentation_num, strength))
                    
                    with open(save_dir+"output.txt", "a") as fi:    
                        print('beta: ',strength)
                        print('beta: %.6f'%(strength), file=fi)
                        print('RMSE: ',RMSE_1)
