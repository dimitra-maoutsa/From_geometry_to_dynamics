...
def _compute_bridge(tindx,
    ...
    with threadpool_limits(limits=1):
        ...
        try:
            ...
            #     plt.show()
            #     plt.close()
            return tindx, Fc_slice, gfFc_slice, True
...
def run_bridge_augmentation_parallel(f_est, bridge_type, ksteps, N, M, reps,aug_num,
                                     strength=1, dt=0.01,savetitle=''):
    """
    ...
    backends_for_paral = ["threading", "loky", "loky", "loky", "loky", "loky",
                          "loky", "loky", "loky", "loky", "loky","loky", "loky",
                          "loky", "loky", "loky","loky", "loky", "loky", "loky", "loky"]
    ...
    with tqdm_joblib(tqdm(total=len(indices), desc="bridges", ncols=100)) as _:
        results = Parallel(n_jobs=n_jobs, backend=the_backend)(
            delayed(_compute_bridge)(i, timegrid_obs, Ob, bridge_type, f_est, g,
                ksteps, N, M, dt,distances=distances, epsilon_ball=epsilon_ball,
                strength=strength)  for i in indices )

    # ------------------------------------------------------------------
    # copy slices back into BALL / gbALL in the *main* process
    #    (avoids concurrent writes & shared-memory headaches)
    # ------------------------------------------------------------------
    for tindx, Fc_slice, gfFc_slice, valid in results:
        if not valid:
...
#seed = 10
seeds = [ 1,2,3]#,11,12,13,14] #12 sta 200 13, 14  ,11,12,13,14, 13, ,14 
for seed in seeds:
    # obs dens 200, 300, 400 at 0.15 k 0.25
    obs_denz = [ 280]#, 160, 200, 240, 280] #, 240, 280
    for obi, obs_dens in enumerate(obs_denz[::-1]):
        ...
        # save_dir = '/Users/dimi/Downloads/my stuff/geometric/new_results/'+first_augm+'Mu05Geodesic_augmentation_'+dsystem+'_noise_%.2f_obs_dens_%d_sim_time_%d_seed_%d/'%(g, obs_dens, T, seed)
        # save_dirplots = '/Users/dimi/Downloads/my stuff/geometric/new_results/'+first_augm+'Mu05Geodesic_augmentation_'+dsystem+'_noise_%.2f_obs_dens_%d_sim_time_%d_seed_%d/plots/'%(g, obs_dens, T, seed)
        
        save_dir = '/Volumes/KINGSTON/geometric_more/convergence/'+first_augm+'Geodesic_augmentation_'+dsystem+'_noise_%.2f_obs_dens_%d_sim_time_%d_seed_%d/'%(g, obs_dens, T, seed)
        save_dirplots = '//Volumes/KINGSTON/geometric_more/convergence/'+first_augm+'Geodesic_augmentation_'+dsystem+'_noise_%.2f_obs_dens_%d_sim_time_%d_seed_%d/plots/'%(g, obs_dens, T, seed)
        ...
        dt = h
        augmentation_num = 0
        strengths = [0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5,0.5, 0.5, 0.5, 0.5, 0.5,0.5]
        # if obs_dens==50 and obi==0:
        #     bridge_types = ['normal']*10
        # else:
        bridge_types = [first_augm, 'geodesic', 'geodesic',  'geodesic', 'geodesic','geodesic', 
                        'geodesic', 'geodesic', 'geodesic', 'geodesic', 'geodesic',
                        'geodesic', 'geodesic', 'geodesic', 'geodesic',  'normal']
        Ns = [100]*20
        epsilon_ball = 0.75
        n_sparsegp = 200
        f_est = None
        ...
        while augmentation_num<16:
            ...
            if augmentation_num==0:
                ...
                BALL, gbALL = run_bridge_augmentation_parallel(f_est, bridge_type, ksteps, 
                                                              N, M, reps,
                                                              aug_num=augmentation_num ,
                                                              strength=strength,
                                                              dt = dt,
            ...
            if OPTIMIZE_LENGTHSCALES:
                ...
                def run_likelihood_grid(Sp, n_sparsegp, BALL2, gbALL2,l1s,
                                        l2s, tmax=1000, n_jobs=8, save_dir=None):
            ...
            to_sav['strengths'] = strengths
            if True:
                to_sav['F_1'] = F_1
            to_sav['Wm_1'] = Wm_1
            to_sav['Wstd_1'] = Wstd_1
            to_sav['birdge_types'] = bridge_types
            ...
            plot_estimated_vector_field_and_invariant_density(xt, ztrue, mu_1, Ob,  savetitle='_augm%d'%augmentation_num)
